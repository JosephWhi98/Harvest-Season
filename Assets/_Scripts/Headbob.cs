...
public class Headbob : MonoBehaviour
{
    ...
    {
        while (true && !GameManager.Instance.GameOver)
        {
            if (playerController.canMove)
            {
                if (PlayerController.Instance.moveInput.x != 0 || PlayerController.Instance.moveInput.y != 0) //moving
                {
                    audioSource.volume = playerController.isRunning ?.3f : 0.15f; 
                    audioSource.pitch = Random.Range(0.9f, 1.1f);

                    if(footstepClip) 
                        audioSource.PlayOneShot(footstepClip);

                    yield return new WaitForSeconds(playerController.isRunning ? 0.5f : 0.8f);
                }
            }
            yield return null;
        }
